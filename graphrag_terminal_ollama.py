...
community reports are first scored for relevance by a local Ollama LLM, then the
highest-scoring reports are passed as context to the answer LLM.
...
DEFAULT_SCORER_MODEL = DEFAULT_OLLAMA_MODEL
DEFAULT_ANSWER_MODEL = DEFAULT_OLLAMA_MODEL
...
class GraphRAGConfig:
    ...
    Attributes:
        ...
        request_timeout: Reserved timeout value for parity with the original CLI.
...
class OllamaClient:
    """Small client wrapper for local Ollama chat completions."""

    def __init__(self, timeout: int = 90) -> None:
        """Initialize the Ollama client.

        Args:
            timeout: Reserved timeout value for CLI compatibility.
        """
    ...
    def chat(
        ...
        Args:
            ...
            response_format: Optional JSON response format request.
...
def strip_thinking_tags(text: str) -> str:
    """Remove Qwen-style thinking blocks from an Ollama response."""
...
def score_report(
    ...
    Args:
        ...
        query: User query.
        model: Ollama model used for scoring.
...
def answer_from_reports(
    ...
    Args:
        ...
        model: Ollama model used for answer synthesis.
        max_report_chars: Maximum report characters per selected report.
...
class GraphRAGAnswerer:
    ...
    def __init__(self, config: GraphRAGConfig) -> None:
        ...
        self.config = config
        self.client = OllamaClient(timeout=config.request_timeout)
